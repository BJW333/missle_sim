...
class FighterEnv:
    ...
    def _get_obs(self):
        """Build 17D observation vector with tactical features."""
        ...
        los_unit = r / max(R, 1e-6)  # Unit vector TO missile
        fwd = self.fighter_vel / max(speed, 1e-6)  # Fighter heading
        
        # Perpendicularity: 1.0 = perfect beam, 0.0 = nose-on/tail-on
        alignment = np.dot(fwd, los_unit)
        perpendicularity = 1.0 - abs(alignment)
        
        # Closing rate (positive = missile getting closer)
        closing_rate = -np.dot(v_rel, los_unit)
        
        # Missile in front or behind? (positive = in front)
        missile_ahead = alignment
        ...
        obs = np.concatenate([
            ...
            np.array([
                ...
                # === NEW FEATURES ===
                perpendicularity,           # 0-1, how well beaming
                closing_rate / 500.0,       # normalized closing speed
                missile_ahead               # -1 to 1, missile relative bearing
            ], dtype=float)
        ])
    ...
    def step(self, action, t):
        ...
        # =================================================================
        # SIMPLIFIED REWARD FUNCTION - Focus on what matters
        # =================================================================
        reward = 0.0
        ...
        if alt_min <= alt <= alt_max:
            ...
            if alt < alt_min:
                deficit = (alt_min - alt) / 1000.0
            else:
                deficit = (alt - alt_max) / 1000.0
            reward -= 20.0 * self.dt * deficit
        
        # --- 3. PERPENDICULARITY REWARD (the key insight) ---
        fwd = self.fighter_vel / max(fighter_speed, 1e-6)
        los_unit = (self.missile_pos - self.fighter_pos) / max(R, 1e-6)
        perpendicularity = 1.0 - abs(np.dot(fwd, los_unit))
        
        if dist_km < 15:
            reward += perpendicularity * 15.0 * self.dt
