...
class FighterEnv:
    ...
    def step(self, action, t):
        ...
        # =====================================================================
        # 1. ALTITUDE COMPLIANCE (Primary constraint - must be respected)
        # =====================================================================
        alt_min, alt_max = self.alt_band  # 9000, 13000
        
        # Strong reward for staying in band
        if alt_min <= alt <= alt_max:
            reward += 5.0 * self.dt  # Good boy points for proper altitude
        
        # SEVERE penalties for leaving the band - scaled quadratically
        if alt < alt_min:
            deficit_km = (alt_min - alt) / 1000.0
            # Quadratic penalty: small violations = small penalty, big = huge
            reward -= 50.0 * self.dt * (deficit_km ** 2)
            ...
            if alt < 4000:
                reward -= 300.0 * self.dt  # Getting close to termination
        
        if alt > alt_max:
            excess_km = (alt - alt_max) / 1000.0
            reward -= 50.0 * self.dt * (excess_km ** 2)
            if alt > 15000:
                reward -= 100.0 * self.dt
        
        # =====================================================================
        # 2. FLIGHT ATTITUDE CONSTRAINTS (No unrealistic flying)
        # =====================================================================
        vert_speed = abs(self.fighter_vel[2])
        
        # Vertical rate limits (real fighters: ~150 m/s max sustained)
        if vert_speed > 80:  # Start penalizing earlier
            excess = (vert_speed - 80) / 50.0
            reward -= 20.0 * self.dt * (excess ** 2)
        
        if vert_speed > 150:  # This is really excessive
            reward -= 50.0 * self.dt
        
        # Flight path angle (pitch) limits
        if fighter_speed > 1e-3:
            fpa_rad = math.asin(np.clip(self.fighter_vel[2] / fighter_speed, -1, 1))
            fpa_deg = math.degrees(abs(fpa_rad))
            
            # Penalize steep climbs/dives
            if fpa_deg > 20:  # Start earlier
                excess = (fpa_deg - 20) / 20.0
                reward -= 15.0 * self.dt * (excess ** 2)
            
            if fpa_deg > 45:
                reward -= 50.0 * self.dt
            
            if fpa_deg > 60:  # Nearly vertical = very wrong
                reward -= 100.0 * self.dt
        
        # =====================================================================
        # 3. HORIZONTAL MANEUVERING REWARD (What we actually want)
        # =====================================================================
        # Compute horizontal component of acceleration
        a_horizontal = np.array([a_cmd[0], a_cmd[1], 0.0])
        a_horiz_mag = np.linalg.norm(a_horizontal)
        a_vert_mag = abs(a_cmd[2])
        
        # Reward horizontal maneuvering (this is realistic evasion)
        if dist_km < 15:
            horiz_g = a_horiz_mag / 9.81
            reward += horiz_g * 2.0  # Reward horizontal turns
            
            # Penalize vertical-dominated maneuvers
            if a_vert_mag > a_horiz_mag and a_vert_mag > 2 * 9.81:
                reward -= 5.0 * self.dt
        
        # =====================================================================
        # 4. TACTICAL REWARDS (Actual evasion goals)
        # =====================================================================
        # Survival reward (only if flying properly)
        if alt_min <= alt <= alt_max:
            reward += 1.0 * self.dt
        
        # Distance-based rewards (smaller magnitude - don't overshadow constraints)
        if dist_km > 15:
            reward += 3.0
        elif dist_km > 10:
            reward += 1.5
            ...
            # Close range danger
            reward -= 5.0 * (5 - dist_km) / 5
        
        # Beaming reward (perpendicular to missile) - good tactic
        if dist_km < 15 and m_speed > 1:
            fwd = self.fighter_vel / max(fighter_speed, 1e-3)
            cos_aspect = abs(np.dot(fwd, los))
            beam_quality = 1.0 - cos_aspect  # 1.0 = perfect beam, 0 = nose-on/tail-on
            reward += beam_quality * 3.0
        ...
        if R < self.kill_radius:
            reward -= 300.0  # Reduced from 500 - altitude violations should hurt more
            terminated = True
        
        # Altitude floor violation (already set terminated above at 3000m)
        if alt < 3000.0:
            reward -= 400.0  # WORSE than getting hit - don't dive!
        
        # Time limit with conditional bonus
        truncated = self.step_count >= self.max_steps
        if truncated:
            # Only get survival bonus if you stayed in the altitude band most of the time
            if alt_min <= alt <= alt_max:
                reward += 50.0  # Reduced bonus
            else:
                reward += 10.0  # Minimal bonus if you cheesed it
...
def train_bc(policy, env, device, n_episodes=200, epochs=10, batch_size=4096):
